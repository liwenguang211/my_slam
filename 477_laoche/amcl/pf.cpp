...
//printf("当前粒子的置信度=%f",my_real_time_data.score);
 ...
 // if(my_real_time_data.amcl_laser_prop>1.0) my_real_time_data.amcl_laser_prop =0.99;
 // printf("当前粒子total=%f的amcl_laser_prop=%f\n",total,my_real_time_data.amcl_laser_prop);
  return;
}
...
/*	pf_vector_t pose_mean;
  ...
  if(update == 1)
  {pf_init_pose_cov.m[0][0] = delta_covx;//delta_covx*cos(theta_robot)+delta_covy*sin(theta_robot);////0.15
  pf_init_pose_cov.m[1][1] = delta_covy;//delta_covx*sin(theta_robot)+delta_covy*cos(theta_robot);////0.15
  pf_init_pose_cov.m[2][2] = 0.02;
  }
  else if(update == 2)
...
//  w_diff = 0.0;
  ...
  if(w_diff>0&&w_diff<0.1) {printf("!!!!!!w_diff=%f",w_diff);w_diff = 0.1;}
  //if(update == 5)
//	w_diff = 0.5;
