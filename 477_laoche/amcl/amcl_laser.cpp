...
double AMCLLaser::LikelihoodFieldModel(AMCLLaserData *data, pf_sample_set_t* set)
{
 ...
 // printf("运行LikelihoodFieldModel模型函数\n");
  // Compute the sample weights
      step = (data->range_count - 1) / (self->max_beams - 1);
      real_partilces = (float)data->range_count/((float)step);
  for (j = 0; j < set->sample_count; j++)
  {
    sample = set->samples + j;
    pose = sample->pose;
...
//	printf("运行LikelihoodFieldModel模型函数range_count=%d,max_beams=%d,step=%d\n",data->range_count,self->max_beams,step);
	...
	//printf("step=%d第%d个距离%f对应角度%f>>>>>\n",step,i,obs_range,obs_bearing);
      ...
      if(obs_range >= (data->range_max-0.1))
	{
	/*i++;
	obs_range = data->ranges[i][0];
      obs_bearing = data->ranges[i][1];
	while(obs_range >= (data->range_max-0.1)&&(i < data->range_count))
	{
	i++;
	obs_range = data->ranges[i][0];
      obs_bearing = data->ranges[i][1];
	}
	step = (data->range_count - i - 1) / (self->max_beams - cur_sample_laser_count - 1);
	if(step < 1)     step = 1;*/
	//printf("重新计算激光i=%d处的step=%d\n",i,step);
	//if(i+step < data->range_count)
        continue;
	}
