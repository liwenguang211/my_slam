...
bool AMCLOdom::UpdateAction(pf_t *pf, AMCLSensorData *data)
{
  ...
  case ODOM_MODEL_DIFF_CORRECTED:
  {
    ...
    inverse_flag=angle_diff(atan2(ndata->delta.v[1], ndata->delta.v[0]),
                              old_pose.v[2]);
   // if(fabs(inverse_flag)>(M_PI*0.5))
   // delta_rot1 = inverse_flag;
   // else
    {
    if(sqrt(ndata->delta.v[1]*ndata->delta.v[1] + 
            ndata->delta.v[0]*ndata->delta.v[0]) < 0.001)
      delta_rot1 = 0.0;
    else
      delta_rot1 = inverse_flag;
    }
	//if(fabs(delta_rot1)>(M_PI*0.5)) delta_rot1=M_PI;
...
//////大于pi/2后，我们就认为机器人反向运动，此时将delta_rot1设置为0
    ...
    for (int i = 0; i < set->sample_count; i++)
    {
      ...
      // Sample pose differences
      delta_rot1_hat = angle_diff(delta_rot1,
                                  pf_ran_gaussian(sqrt(alpha1t*delta_rot1_noise*delta_rot1_noise +
                                                       alpha2t*delta_trans*delta_trans)));
      delta_trans_hat = delta_trans - 
              pf_ran_gaussian(sqrt(alpha3t*delta_trans*delta_trans +
                                   alpha4t*delta_rot1_noise*delta_rot1_noise +
                                   alpha4t*delta_rot2_noise*delta_rot2_noise));
	//if(fabs(delta_rot1)>(M_PI*0.5)) delta_trans_hat = -delta_trans_hat;
      delta_rot2_hat = angle_diff(delta_rot2,
                                  pf_ran_gaussian(sqrt(alpha1t*delta_rot2_noise*delta_rot2_noise +
                                                       alpha2t*delta_trans*delta_trans)));
